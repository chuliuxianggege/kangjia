;(function($){
			...
			//3加载到innerHTML里
	function ajaxnewpro(eleclass,ajaxurl){
		this.init(eleclass,ajaxurl);
	}
	ajaxnewpro.prototype={
		constructor:ajaxnewpro,
		init:function(eleclass,ajaxurl){
			// console.log($li[0]);
			//1ajax抓数据成功后且分别传入img,span123;
			$.ajax({url:ajaxurl,success:function(res){			
					res=JSON.parse(res);
					var $ul=$(eleclass);
	...
	$.fn.extend({
		ajaxnewpro:function(eleclass,ajaxurl){
			// console.log(1);
			return new ajaxnewpro(eleclass,ajaxurl);}
		}
	)
})(jQuery)
