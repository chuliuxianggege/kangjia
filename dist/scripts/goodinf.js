...
define(["jquery"], function ($) {
	...
	goodsMagnify.prototype = {
		...
		liErgodic: function liErgodic(index, item) {
			$(item).on("mouseenter", $.proxy(this.lichange, this));
		},
		lichange: function lichange(e) {
			var e = e || window.e;
		...
		mouesenterEvt: function mouesenterEvt() {
			$(this.$video).css({
				display: 'block'
			});
			$(this.$right).css({
		...
		mouesleaveEvt: function mouesleaveEvt() {
			$(this.$video).css({
				display: 'none'
			});
			$(this.$right).css({
		...
		mouesmoveEvt: function mouesmoveEvt(event) {
